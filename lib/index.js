...
var Arena = function (params) {
   ...
   * @type {Object}
   */
  this.settings = _.defaults(params, {
...
Arena.prototype._initListeners = function() {
  ...
  this.settings.container.addEventListener('DOMMouseScroll', this._onMouseScroll.bind(this), false ); // firefox
  ...
  if (intersects.length > 0) {
    ...
    // Mark some polys as not walkable
      ...
      if (intersects[0].object && intersects[0].object) {
        ...
        for (var i = 0; i < intersects.length && !actionTaken; i++) {

          // maybe an entity ?
          entity = Utils.childOf(intersects[i].object, Entity);
          ...
          if (entity) {
            ...
            for (var s = 0; s < character.state.spells.length; s++) {
              ...
              if (spell.canHit(character, entity)) {

                // character.lookAt(entity.position);
                character.cast(spell, entity);
                actionTaken = true;

                debug('cast %o on %o', spell, entity);

                break;

              } else {
                debug('cannot cast %o on %o', spell, entity);
              }
            }

          // it's an interactive object
          } else if (intersects[i].object.parent && intersects[i].object.parent.parent &&
              intersects[i].object.parent.parent instanceof InteractiveObject) {

            if (intersects[i].object.parent.parent.isNearEnough(character)) {
              self.startInteraction(intersects[i].object.parent.parent);
              actionTaken = true;
...
Arena.prototype._prepareEntity = function(entity) {
  ...
  // has a radius helper
  entity.radiusHelper = new THREE.Mesh(new THREE.CylinderGeometry(1, 1, 1, 32, 32, true), self.commonMaterials.entityHelpers);
  entity.radiusHelper.scale.set(entity.state.radius, entity.state.height, entity.state.radius);
  ...
  self.on('update', function(game){
    ...
    entity.lifebar.position.y = box.max.y - box.min.y;
...
Arena.prototype.update = function() {
  ...
  if (this.entities.length > 0) {
    ...
    if (cameraType === settings.CAMERA_MANUAL) {
      ...
      this.camera.position.z = this.entity.position.z + 100;
    }
  ...
  _.each(this.entities, function(character){
