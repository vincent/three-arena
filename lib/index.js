...
Arena.prototype.updateZoneSelector = function(event) {
  ...
  if (this._waitForZoneSelection) {
    ...
    if (intersects.length > 0) {
      ...
      this._currentZoneSelector.position.y += 1;
      this._currentZoneSelector.material.opacity = 0.9;
      this._currentZoneSelector.visible = true;
      return;
    }
  }
  this._currentZoneSelector.visible = false;
  this._currentZoneSelector.material.opacity = 0.001;
};
...
Arena.prototype.update = function() {
  ...
  self._timings.lastDuration_taevents = window._ta_events.emitWithConstraints({
    debug: false,
    each: 1,
    total: 16.0
  }, 'update', this);
  ...
  // update event
  self._timings.lastDuration_updateevent = this.emitWithConstraints({
    debug: false,
    each: 1,
    total: 16.0
  }, 'update', this);
  ...
  // entities behaviours
  self._timings.lastDuration_behaviours = this.emitWithConstraints({
    debug: false,
    each: 1,
    total: 16.0
  }, 'update:behaviours', this);
