...
Arena.prototype._onDocumentMouseUp = function(event) {
  ...
  if (intersects.length > 0) {
    ...
    debug('intersect at %o %o', ipos.x, ipos.y, ipos.z );
    ...
    // Mark some polys as not walkable
      ...
      if (character) {
        debug('find a path between %o and %o', character.position, ipos);
...
Arena.prototype.addObstacle = function(position, radius, flag) {
  ...
  if (this.settings.showObstacles) {
    var obsctacle = new THREE.Mesh( new THREE.PlaneGeometry(radius.x, radius.y, radius.z, 1, 1, 1), new THREE.MeshBasicMaterial({ color: 0xff0000 }) );
